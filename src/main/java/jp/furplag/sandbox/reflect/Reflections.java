...
public interface Reflections {
   ...
   * <p>
   * If there are no annotations <em>present</em> on this element, the return value is {@link Collections#emptySet()} .
   * </p>
   *
  ...
  static Class<?> getClass(final Object mysterio) {
    return mysterio == null ? null : (mysterio instanceof Class) ? ((Class<?>) mysterio) : mysterio.getClass();
  }
  ...
  private static boolean notAssignable(final Class<?> typeOfFiled, final Class<?> typeOfValue) {
    return typeOfFiled == null || (typeOfFiled.isPrimitive() && typeOfValue == null) || (Number.class.isAssignableFrom(ClassUtils.primitiveToWrapper(typeOfFiled)) && Character.class.equals(ClassUtils.primitiveToWrapper(typeOfValue)));
  }
}
