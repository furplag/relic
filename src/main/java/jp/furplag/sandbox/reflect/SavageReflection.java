...
public interface SavageReflection {
  ...
  private static Stream<Field> readFields(final Object mysterio, final String... excludes) {
    return Streamr.stream(Reflections.getFields(mysterio)).filter(mysterio instanceof Class ? Reflections::isStatic : Predicate.not(Reflections::isStatic)).filter((t) -> Trebuchet.Predicates.orNot(t, (x) -> !Streamr.collect(excludes).contains(x.getName())));
  }
  ...
  private static Map<String, Object> mappingFields(final Stream<Field> fields, final Object mysterio) {
    return Streamr.stream(fields).collect(LinkedHashMap::new, (map, t) -> map.putIfAbsent(t.getName(), readField(mysterio, t)), LinkedHashMap::putAll);
  }
