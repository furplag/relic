...
import jp.furplag.sandbox.trebuchet.Predicates;
...
public final class TheUnsafe {
  ...
  private static final Functions.Tri<Class<?>, Class<?>, UnsafeWeaver.Prefix, ? extends Map.Entry<Class<?>, MethodHandle>> pairGenerator =
    (x, y, z) -> Map.entry(y, Trebuchet.orNull(x, UnsafeWeaver.getFormattedMethodName(y, z), UnsafeWeaver.getMethodType(y, z), UnsafeWeaver::getMethodHandle));

  /** internal snippet for cast the value type . */
  private static final Functions.Bi<Class<?>, Object, Object> primitivatorOrigin =
  ...
  private static final Predicates.Tri<Object, Field, Object> setOrigin =
    ((Predicates.Tri<Object, Field, Object>) Reflections::isAssignable)
      .and((t, u, v) -> Trebuchet.orNot(t, u, v, (x, y, z) -> {theUnsafe().setInternal(x, y, z); return Objects.equals(primivatior(y.getType(), z), get(x, y));}));
   ...
   *//* @formatter:off */
  private TheUnsafe() {
    final Class<?> unsafeClass = Trebuchet.orNull("sun.misc.Unsafe", Class::forName);
    theUnsafe = Trebuchet.orNull(unsafeClass, "theUnsafe", (x, y) -> Reflections.conciliation(x.getDeclaredField(y)).get(null));

    staticFieldBase = Trebuchet.orNull(unsafeClass, "staticFieldBase", MethodType.methodType(Object.class, Field.class), UnsafeWeaver::getMethodHandle);
    staticFieldOffset = Trebuchet.orNull(unsafeClass, "staticFieldOffset", MethodType.methodType(long.class, Field.class), UnsafeWeaver::getMethodHandle);
    objectFieldOffset = Trebuchet.orNull(unsafeClass, "objectFieldOffset", MethodType.methodType(long.class, Field.class), UnsafeWeaver::getMethodHandle);
  ...
  private static Object primivatior(final Class<?> fieldType, final Object value) {
    return Trebuchet.orNull(fieldType, value, primitivatorOrigin);
  }
  ...
  public static boolean set(final Object mysterio, final Field field, final Object value) {
    return Trebuchet.orNot(mysterio, field, value, setOrigin);
  }
  ...
  private Object fieldBase(Object mysterio, Field field) {
    return Trebuchet.orNull(mysterio, field, (o, f) -> Reflections.isStatic(f) ? staticFieldBase.invoke(theUnsafe, f) : o);
  }
  ...
  private long fieldOffset(Field field) {
    return (long) Trebuchet.orElse(field, (t) -> (Reflections.isStatic(t) ? staticFieldOffset : objectFieldOffset).invoke(theUnsafe, t), () -> invalidOffset);
  };
  ...
  private Object getInternal(Object mysterio, Field field) {
    ...
    return !(Reflections.isAssignable(mysterio, field) && (Reflections.isStatic(field) || !(mysterio instanceof Class))) ? null :
      Trebuchet.orNull(mysterio, field, (o, f) -> gettings.getOrDefault(f.getType(), gettings.get(Object.class)).invoke(theUnsafe, fieldBase(o, f), fieldOffset(f)));
    // @formatter:on
  }
  ...
  private void setInternal(Object mysterio, Field field, Object value) {
    Trebuchet.orNone(mysterio, field, value, (_mysterio, _field, _value) -> settings.getOrDefault(_field.getType(), settings.get(Object.class)).invoke(theUnsafe, fieldBase(_mysterio, _field), fieldOffset(_field), primivatior(_field.getType(), _value)));
  }
}
